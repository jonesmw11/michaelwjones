...
def carr_madan_batch(strikes, ttms, rates, divs, C, G, M, Y, S0,
    ...
    #Define strikes, ttm, rate and div yields - all relevant in solving equations
    ...
    # Denominator of psi(v) is contract-independent
    denom = alpha**2 + alpha - v**2 + 1j * (2 * alpha + 1) * v

    #Now the actual carr-madan process - see equations for references
...
def carr_madan_batch_gh(strikes, ttms, rates, divs, a, b, d, nu, S0,
    ...
    #made it to the exponent in this section
    for T in np.unique(ttms):
        sel = ttms == T
        r, q = rates[sel][0], divs[sel][0]
...
def varphi_cir(u, t, kappa, eta, lam, y0=1.0):
    ...
    ekt = np.exp(gam * t)
    # exp(kappa^2 eta t / lam^2) * exp(2 y0 iu / (kappa + gam coth(gam t/2)))
    #   / (cosh(gam t/2) + kappa sinh(gam t/2)/gam)^{2 kappa eta / lam^2}
    coth = (ekt + 1) / (ekt - 1)
...
def implied_vol(mkt, K_, T_, r, q, sigma0, spot, tol=1e-9, max_iter=100):
    ...
    for _ in range(max_iter):
        ...
        # The degenerate cases the sigma_0 = 0.20 run kept reaching.
...
def fit_bs(o, spot=None, sigma0=IV_SEED, tol=1e-12, max_iter=100):
    ...
    #This helps with boundary conditions and actually making the algo
    #But note the max iterations
    sigma, h = sigma0, 1e-6
    for _ in range(max_iter):
        gs = g(sigma)
        if abs(gs) < tol:
...
def fit_levy(spec, o, spot=None, maxiter=4000):
    ...
    # Extracted once, outside the objective, since the optimiser evaluates it
    # thousands of times.
    ...
    # Every start is run to convergence and only the best kept,
    best = None
...
def density_moments(T_, name, params):
    ...
    #Meixner is unique cause we have the closed form density
    if name == "Meixner":
        a, b, d = params
...
for expiry, g in sorted(opts.groupby("expiry_date"),
    ...
    #For each levy item now, 
    for name, spec in LEVY.items():
        sub = dict(spec)
        sub["starts"] = [list(levy_fits[name])] + spec["starts"][:3]

        #Run respective fit levy
...
for name, (obj, pricer) in BNS_MODELS.items():
    ...
    #we remembered to save the fits here
...
def calibrate_sv(chain_tag, o, spot, levy_name, tc_name, levy_starts,
    ...
    #Now noting the relevant objective function, minimise it
    ...
    # The fitted prices, and the error measures taken from them.
    ...
    #Some extra stats from the chain - get RMSE
...
def sv_moments(T_, levy_name, clock, lp, tp):
    # The Levy exponent and the clock's Laplace exponent for this pairing.

    #Get the characteristic and characteristic exponent functions
    psi, _ = LEVY_PSI[levy_name]
    varphi, _ = TIME_CHANGE[clock]

    # The cumulant generating function of the log return. (Recentered)
    def logphi(u):
        v = levy_sv_char_fn(np.array([u], dtype=complex), T_, R, Q,
                            psi, lp, varphi, tp, S0)[0]
        return np.log(v * np.exp(-1j * u * np.log(S0)))

    # One estimate per step size, skipping any that fail outright.
    est = []
    for h in H_LADDER:
        try:
    ...
    # The first pair of consecutive steps to agree. Returning NaN when no pair
    def settle(i):
...
for _, r in sv.iterrows():
    ...
    if not np.isfinite(VX):
        ...
        continue
    # eta plays the role of a/b and kappa that of lambda, so the two branches
    # are the same formula with the clock's own parameter names.
    if clock == "CIR":
