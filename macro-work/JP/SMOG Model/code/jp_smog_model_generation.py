# Fit the JP SMOG state-space model on historical Japanese data.
#
# Builds model inputs from the Generation sheet of JP SMOG Model Inputs.xlsx,
...
# Signals: log GDP, unemployment, inflation, and labour-cost growth.
# Japan retains two unemployment-gap lags and its labour-cost equation.
# Australia supplies the Python fitting, updates, and diagnostic workflow.
...
try:
    ...
    HERE = next(
        (candidate for parent in (Path.cwd(), *Path.cwd().parents)
         for candidate in (
             parent,
             parent / "macro-work" / "JP" / "SMOG Model",
             parent / "michaelwjones" / "macro-work" / "JP" / "SMOG Model",
         ) if (candidate / "code" / "JP SMOG Model Inputs.xlsx").is_file()),
        None,
    )
    if HERE is None:
        raise FileNotFoundError("Open the repository before running JP SMOG cells")
IR_DIR = HERE  # Keep all files local.
INPUTS_XLSX = IR_DIR / "code" / "JP SMOG Model Inputs.xlsx"
GENERATION_SHEET = "Generation"
UPDATE_SHEET = "Update"
OUT_DIR = HERE / "output"
PARAMETERS_FILE = HERE / "jp_smog_fitted_parameters.json"
FIT_HISTORY_FILE = OUT_DIR / "jp_smog_fit_history.csv"
FIT_CONTRIBUTIONS_FILE = OUT_DIR / "jp_smog_fit_likelihood_by_quarter.csv"
...
# Read matching input layouts.
HISTORICAL_COLUMNS = [
    "date", "real_gdp_sa", "cpi_core_sa", "import_price_deflator",
    "labour_sa", "unemployment", "expectations_tankan",
]
MODEL_INPUT_COLUMNS = HISTORICAL_COLUMNS.copy()
STATE_NAMES = ["gap", "gap_1", "gap_2", "y_star", "y_star_1",
               "u_star", "u_star_1", "u_star_2"]
PARAM_NAMES = [
    "beta1", "beta2", "beta3", "beta4",
    "eps1", "eps2", "eps3", "eps4", "eps5", "eps6", "eps7",
    "g_star", "gamma", "lambda1", "lambda2", "lambda3",
    "phi1", "phi2", "psi", "rho1", "rho2",
]
...
def build_dataset(boost=1):
    ...
    d["gdp"] = np.log(raw["real_gdp_sa"])
    cpi = raw["cpi_core_sa"]
    d["inflation"] = pcy(cpi)
    d["expectations"] = raw["expectations_tankan"]
    ...
    d["covid_d"] = 0.0  # No separate COVID coefficient.
    ...
    d["d_it"] = (d.index > pd.Period("2012Q4", freq="Q")).astype(float)
...
# Set the eight starting states from the historical trend estimates.
def make_svec(d, first=ESMPL_FIRST):
    ...
    ys = d["ham_y_star"]
    us = d["ham_u_star"]
    # Order eight initial states.
    return np.array([
        g[p0 + 2], g[p0 + 1], g[p0],
        ys[p0 + 2], ys[p0 + 1],
        us[p0 + 2], us[p0 + 1], us[p0],
    ])

# %% State-space model
# Define the JP SMOG observation, transition, and variance equations.
class SmogJP(MLEModel):
    # Define JP model equations.
    ...
    def __init__(self, endog, exog, svec, prior_var=0.0, init_mode="direct"):
        ...
        miss_u = exog[["u1", "u2"]].isna().any(axis=1)
        ...
        # Mask labour-cost regressors.
        miss_nulc = exog[["pie", "pi1", "u1"]].isna().any(axis=1)
        endog.loc[miss_nulc, "delta_nulc"] = np.nan

        # Count partial quarters.
        self.n_partial = int(((miss_y | miss_u | miss_pi | miss_nulc) & endog.notna().any(axis=1)).sum())
        ...
        super().__init__(endog, k_states=8, k_posdef=3,
                         initialization="known", constant=self.svec,
                         stationary_cov=self.prior_var * np.eye(8))
        ...
        self["state_intercept"] = np.zeros((8, 1))


    # %% Parameter labels
    # Expose coefficient names.
    @property
    def param_names(self):
        return PARAM_NAMES

    # %% Explicit starting values
    # Require data-based guesses.
    @property
    def start_params(self):
        raise NotImplementedError("pass start_params explicitly")

    # %% Regressor selection
    # Retrieve named regressor.
    def _col(self, name):
    ...
    def update(self, params, **kwargs):
        ...
        self["design", 1, 7] = -p["rho2"]
        # Bound labour-gap loading.
        labour_loading = 8.0 + 1.0 / (1.0 + np.exp(-p["lambda2"]))
        self["design", 3, 6] = -labour_loading
        ...
        # Set signal noise.
        self["obs_cov"] = np.diag([p["eps1"] ** 2, p["eps2"] ** 2, p["eps3"] ** 2, p["eps4"] ** 2])
...
def save_results(out, out_dir=OUT_DIR, name="smog_jp"):
...
def initial_parameters(d):
    ...
    # %% Starting regression helper
    # Fit initial relationships.
    ...
    (rho1, rho2, lambda3), unemployment_noise = regression(
        [unemployment_gap, unemployment_gap.shift(1), unemployment_gap.shift(2), gap]
    )
    post = w.loc["2013Q1":]
...
def fit_parameters(mod, d):
    ...
    scale = np.array([1, 1, 1, 1, .01, .1, .3, 5, .01, .005, .1,
                      .01, .01, 5, 1, 20, 1, 1, .02, 1, 1])
    # Set parameter bounds.
    lower = np.array([-3, -3, -3, -10, .00001, .00001, .00001, .00001,
                      .00001, .00001, nairu_noise_floor, -.02, -2, -100,
                      -20, -100, -1.8, -1, -1, -1.8, -1])
    upper = np.array([3, 3, 3, 10, .5, 2, 5, 50, .1, .1, 2, .03,
                      2, 100, 20, -.01, 1.8, 1, 1, 1.8, 1])
    ...
    # %% Accepted iteration callback
    # Record optimizer progress.
    def record_iteration(scaled):
        ...
        if quarterly.shape != (mod.nobs,) or not np.isfinite(quarterly).all():
            raise RuntimeError("Cannot record a finite JP likelihood trajectory")
    ...
    if not np.isclose(history[-1]["log_likelihood"], -result.fun, atol=1e-6):
        raise RuntimeError("Recorded JP likelihood differs from the optimizer result")
...
def show_fit_figures(history, nairu_noise_floor):
    ...
    if trajectory.empty or not set(columns).issubset(trajectory.columns):
        raise ValueError("JP fit history is empty or missing parameters")
    values = trajectory[columns].to_numpy(dtype=float)
    if not np.isfinite(values).all():
        raise ValueError("JP fit history contains non-finite values")
    ...
    if not np.array_equal(iterations, np.arange(len(iterations))):
        raise ValueError("JP fit iterations must start at zero and be consecutive")
    # Compare every coefficient.
    parameter_fig, axes = plt.subplots(7, 3, figsize=(15, 21), sharex=True,
                                      layout="constrained")
    ...
    parameter_fig.suptitle("JP SMOG parameter convergence\n"
    ...
    ax_total.set_title("JP SMOG likelihood convergence", loc="left", fontsize=16)
...
def main():
    ...
    for n, a in zip(STATE_NAMES,
                    svec):
        print(f"  {n:8s} {a:12.6f}")

    # Build state-space model.
    mod = SmogJP(endog, exog, svec)
