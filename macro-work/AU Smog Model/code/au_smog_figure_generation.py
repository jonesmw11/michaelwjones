...
# Load plotting dependencies and locate AU SMOG results.
...
def save_figures(smoothed, filtered, observed_unemployment):
    ...
    # Plot output gap.
    fig, ax = styled_chart(
        "Australia: unemployment-implied output gap",
    ...
    save_chart(fig, FIGURES_DIR / "au_smog_output_gap.png")
    ...
    fig, ax = styled_chart(
        "Australia: unemployment and the NAIRU", "Per cent"
    )
    ...
    ax.set_xlim(dates[0], dates[-1])
    place_labels(fig, ax, sample_end)
    save_chart(fig, FIGURES_DIR / "au_smog_unemployment_and_nairu.png")
    ...
    ax.set_xlim(dates[0], dates[-1])
    place_labels(fig, ax, sample_end)
    save_chart(fig, FIGURES_DIR / "au_smog_unemployment_gap.png")
