...
def load_inputs():
    paths = list((ROOT / "data").glob("*_headline.mat"))
    if len(paths) != 1:
...
def export(path):
    ...
    ax.plot(
        context.index,
        context["headline_yoy"],
        color="#707070",
        lw=1.25,
        alpha=0.8,
        label="Observed headline CPI, year ended",
    )
    if len(finite_core):
    ...
    ax.set_title(f"{diagnostics['country']}: Smoothed Headline MCT")
    ...
    chart_destination = ROOT.parents[3] / "Charts/Current Model Charts/AU - MCT Headline Inflation.png"
    chart_destination.parent.mkdir(parents=True, exist_ok=True)
    shutil.copyfile(chart_path, chart_destination)
    shutil.copyfile(chart_path, ROOT.parents[3] / "Charts/Current Model Charts/AU - MCT Headline Smoothed.png")
    ...
    ax.plot(context.index, context["headline_yoy"], color="#9A9A9A", lw=1.1, alpha=0.75, label="Observed headline CPI, year ended")
    ax.axhspan(2, 3, color="#B5651D", alpha=0.09, label="RBA 2–3% target band")
    ax.axhline(2, color="#B5651D", lw=1.1, ls=":")
    ax.axhline(3, color="#B5651D", lw=1.1, ls=":")
    ax.set_title(f"{diagnostics['country']}: Filtered Headline MCT")
    ax.set_ylabel("Annualized inflation, percent")
    ax.legend(frameon=False)
    fig.tight_layout()
    ...
    shutil.copyfile(filtered_chart_path, ROOT.parents[3] / "Charts/Current Model Charts/AU - MCT Headline Filtered.png")
    ...
    ax.axhline(2, color="#B5651D", lw=1.1, ls=":")
    ax.axhline(3, color="#B5651D", lw=1.1, ls=":")
    ax.set_title(f"{diagnostics['country']}: Persistent headline MCT sector trends")
    ax.set_ylabel("Annualized inflation, percent")
    ...
    shutil.copyfile(
        sector_chart_path,
        ROOT.parents[3] / "Charts/Current Model Charts/AU - MCT Headline Sector Trends.png",
    )
