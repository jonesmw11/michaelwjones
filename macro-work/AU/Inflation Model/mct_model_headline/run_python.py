# Run the self-contained Australia headline MCT estimator.
...
def run(draws=3000, burn=3000, thin=2, seed=2022):
    data = loadmat(ROOT / "data/australia_headline.mat", simplify_cells=True)
    np.testing.assert_allclose(data["weights"].sum(axis=1), 1, atol=1e-12)
    destination = ROOT / "results/python"
    destination.mkdir(exist_ok=True, parents=True)
    tag = f"australia_headline_d{draws}_b{burn}_t{thin}_s{seed}"
    ...
    def progress(iteration, total, elapsed):
        ...
        print(f"Australia headline MCT: {iteration}/{total}; {elapsed:.1f}s", flush=True)
