# Local Australia copy of the published NY Fed independent-sector configuration.
# The time-unit priors are scaled to quarterly observations.
...
class Priors:
    prec_ma: float = 0.1
    nu_lam: float = 12
    s2_lam: float = 0.25**2 / 60 / 4
    nu_gam: float = 60
    s2_gam: float = 1 / 60 / 4
    a_ps: float = (1 - 1 / 16) * 120
    b_ps: float = 120 / 16
...
class MCT:
    ...
    def __init__(self, y, seed=2022, priors=None):
        ...
        if not np.isfinite(self.y).all() or self.y.ndim != 2:
            raise ValueError('This implementation requires a complete quarterly input matrix.')
