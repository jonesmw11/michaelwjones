...
def export(path):
    ...
    original_core = pd.read_csv(
        ROOT.parent / "results/au_inflation_forecast.csv",
        index_col=0,
    )["ex_food_energy_yoy"]
    original_core.index = pd.PeriodIndex(original_core.index, freq="Q").to_timestamp(how="end").normalize()
    original_core = original_core.reindex(dates)
    context = pd.DataFrame({
        "headline_yoy": np.asarray(data["headline_yoy"], dtype=float),
        "official_core_yoy": np.asarray(data["official_core_yoy"], dtype=float),
        "observed_core_yoy": original_core.to_numpy(dtype=float),
        "model_core_input": np.sum(
            np.asarray(data["weights"], dtype=float) * np.asarray(data["y"], dtype=float),
            axis=1,
        ),
    }, index=dates)
    ...
    ax.fill_between(dates, frame.iloc[:, 0], frame.iloc[:, 2], color="#0B6E4F", alpha=0.16, label="Smoothed central 66.7% interval")
    ax.plot(dates, frame.MCT_median, color="#0B6E4F", lw=2.2, label="Smoothed MCT")
    ax.plot(
        context.index,
        context["observed_core_yoy"],
        color="#707070",
        lw=1.25,
        alpha=0.8,
        label="Observed core CPI ex food and energy, year ended",
    )
    if len(finite_core):
        ax.plot(finite_core.index, finite_core, color="#9A9A9A", lw=1.1, label="Official core, year ended")
    ax.axhspan(2, 3, color="#B5651D", alpha=0.09, label="RBA 2–3% target band")
    ax.axhline(2, color="#B5651D", lw=1.1, ls=":")
    ax.axhline(3, color="#B5651D", lw=1.1, ls=":")
    ax.set_title(f"{diagnostics['country']}: Smoothed Core MCT")
    ...
    shutil.copyfile(chart_path, ROOT.parents[3] / "Charts/Current Model Charts/AU - MCT Core Smoothed.png")
    ...
    ax.plot(context.index, context["observed_core_yoy"], color="#9A9A9A", lw=1.1, alpha=0.75, label="Observed core CPI, year ended")
    ax.axhspan(2, 3, color="#B5651D", alpha=0.09, label="RBA 2–3% target band")
    ax.axhline(2, color="#B5651D", lw=1.1, ls=":")
    ax.axhline(3, color="#B5651D", lw=1.1, ls=":")
    ax.set_title(f"{diagnostics['country']}: Filtered Core MCT")
    ax.set_ylabel("Annualized inflation, percent")
    ax.legend(frameon=False)
    fig.tight_layout()
    ...
    shutil.copyfile(filtered_chart_path, ROOT.parents[3] / "Charts/Current Model Charts/AU - MCT Core Filtered.png")

    # =============================================================================
    # %% Export the persistent sector-trend chart
    core_mask = np.asarray(data["core_mask"], dtype=bool)
    fig, ax = plt.subplots(figsize=(13, 7))
    palette = plt.get_cmap("tab20")
    for number, label in enumerate(labels):
        included = bool(core_mask[number])
        ax.plot(
            dates,
            sector_trends[label],
            ...
            lw=1.35,
            ls="-" if included else "--",
            alpha=0.88 if included else 0.7,
            label=label if included else f"{label} (excluded)",
        )
    ax.axhspan(2, 3, color="#B5651D", alpha=0.09, label="RBA 2–3% target band")
    ax.axhline(2, color="#B5651D", lw=1.1, ls=":")
    ax.axhline(3, color="#B5651D", lw=1.1, ls=":")
    ax.set_title(f"{diagnostics['country']}: Persistent MCT sector trends")
    ax.set_ylabel("Annualized inflation, percent")
    ax.legend(frameon=False, bbox_to_anchor=(1.01, 1), loc="upper left", fontsize=8)
    fig.tight_layout()
    sector_chart_path = path.with_name(path.stem + "_sector_trends.png")
    fig.savefig(sector_chart_path, dpi=160)
    plt.close(fig)
    shutil.copyfile(sector_chart_path, path.parent / "latest_sector_trends.png")
    shutil.copyfile(
        sector_chart_path,
        ROOT.parents[3] / "Charts/Current Model Charts/AU - MCT Sector Trends.png",
    )
