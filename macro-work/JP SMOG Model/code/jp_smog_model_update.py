# Extend JP SMOG to new quarters while holding fitted parameters fixed.
#
# Update the Update sheet of code/JP SMOG Model Inputs.xlsx, then run this script. It retains
# the original 1995Q1–2022Q4 estimation data and smooths the extended series
# with the coefficients saved by jp_smog_model_generation.py. It does not fit a
# new parameter vector.

# %% Imports and file paths
# Load dependencies and locate JP SMOG inputs and outputs.
import sys
import argparse
import json
...
import jp_smog_figure_generation as figure_code

HERE = CODE_DIR.parent
PARAMETERS_FILE = HERE / "jp_smog_fitted_parameters.json"
RESULTS_FILE = HERE / "results" / "smog_jp_python.csv"
FILTERED_RESULTS_FILE = HERE / "results" / "smog_jp_filtered_states.csv"
...
def load_fixed_parameters():
    ...
    if not PARAMETERS_FILE.is_file():
        raise FileNotFoundError(
            f"Missing {PARAMETERS_FILE}. Run jp_smog_model_generation.py once first."
        )
    ...
    if (payload.get("parameter_names") != model_code.PARAM_NAMES
        ...
        raise ValueError("Saved parameters do not match this JP SMOG specification")
    if (payload.get("source") != "Python maximum likelihood (L-BFGS-B)"
        ...
        raise ValueError(
            ...
            "run jp_smog_model_generation.py first"
        )
...
def load_latest_inputs():
    ...
    numeric = [
        "real_gdp_sa", "cpi_core_sa",
        "unemployment", "expectations_tankan", "labour_sa", "import_price_deflator",
    ]
...
def main(check_only=False):
    ...
    required = ["y", "u", "pi", "pi_e", "delta_4_pm", "delta_nulc"]
    # Find latest complete quarter.
    complete = latest_data.loc[first_new:, required].dropna()
    if complete.empty:
        raise ValueError("No complete JP quarter after the original estimation sample")
    ...
    if not newer.index.equals(expected) or newer[required].isna().any().any():
        raise ValueError("The new quarterly JP inputs have a gap or missing model values")
    ...
    fitted = model_code.SmogJP(endog, exog, initial_state).smooth(
    ...
    print(f"JP SMOG: {len(out)} quarters through {latest}; fixed parameters unchanged")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Extend JP SMOG using fixed fitted parameters"
    )
